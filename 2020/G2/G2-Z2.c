...
int maxParniSusjed(int i, int j, int n, int matrix[n][n]){
    ...
    int arrIndex = 0;
    int maxParni = 0;
    if(validPos(i+1,j,n))arraySusjedi[arrIndex++] = matrix[i+1][j];
    if(validPos(i-1,j,n))arraySusjedi[arrIndex++] = matrix[i-1][j];
    if(validPos(i,j+1,n))arraySusjedi[arrIndex++] = matrix[i][j+1];
    if(validPos(i,j-1,n))arraySusjedi[arrIndex++] = matrix[i][j-1];
...
int main(){
    ...
    for(int i = 0;i < n;i++){
        for(int j = 0; j < n; j++){
            if(i == j){ // orginalana glavna dijagonala
                dijagonala1 = 10 * dijagonala1 + matrica[i][j];
            }
            if(i+j == n-1){ //orginalna sporedna matrica
